...
window.addEventListener('vff:init', function(event){
	console.log("vff:init", event);
...
window.addEventListener('vff:change', function(event){
	...
	switch (event.target.tagName) {
		...
		default:
			if(event.target.hasAttribute(ATTRIBUTE.DATA)){
				_data[event.target.getAttribute(ATTRIBUTE.DATA)] = event.detail.data;
			}

			if(event.target.hasAttribute(ATTRIBUTE.STYLE)){
				_style[event.target.getAttribute(ATTRIBUTE.STYLE)] = event.detail.data;
			}
			//todo handle elements with same vff-data ot vff-style
			scanSelectFrom();
			//todo handle style
			_data.__style = _style;
